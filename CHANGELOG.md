...
- Endpoint `create_config` emits the new `EvtCreateConfig3` event in addition to `EvtCreateConfigV2`. Both config creation endpoints now emit the same pair of events, so an indexer that follows only one of the two events sees every new config.
