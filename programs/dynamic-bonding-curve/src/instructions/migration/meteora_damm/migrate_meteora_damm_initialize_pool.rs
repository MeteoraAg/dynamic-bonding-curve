...
pub fn handle_migrate_meteora_damm<'info>(
    ...
    let liquidity_distribution = config.get_liquidity_distribution_u64(lp_minted_amount)?;
    migration_metadata
        .set_liquidity_token_minted(ctx.accounts.lp_mint.key(), &liquidity_distribution);
