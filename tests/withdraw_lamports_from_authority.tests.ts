...
import { fundSol, processTransactionMaybeThrow, startTest } from "./utils";
import {
...
describe("Withdraw lamports from authority", () => {
    ...
    it("test withdraw", async () => {
        ...
        withdrawTx.recentBlockhash = (await context.banksClient.getLatestBlockhash())[0];
        withdrawTx.sign(context.payer);
        await processTransactionMaybeThrow(context.banksClient, withdrawTx);

        let postTreasuryBalance = (await context.banksClient.getAccount(treasury)).lamports;

        expect(postTreasuryBalance - preTreasuryBalance).eq(prePoolAuthorityBalance)
    })
});